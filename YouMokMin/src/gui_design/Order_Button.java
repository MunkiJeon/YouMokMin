...
public class Order_Button extends JButton {
	...
	public Order_Button(String text,int R, int G, int B) {
		super(text);
		this.rR = R;
		this.rG = G;
		this.rB = B;
	...
	public Order_Button(String text,int R, int G, int B,int font, int round) {
		super(text);
		this.rR = R;
		this.rG = G;
		this.rB = B;
	...
	protected void paintComponent(Graphics g) {
		...
		if (getModel().isArmed()) {//버튼이 눌리는동안)
			...
			grp.setColor(new Color(210,210,210)); 
		}else { //평소
			R=rR;
			G=rG;
			B=rB;
