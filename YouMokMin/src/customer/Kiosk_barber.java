...
import customer.Kiosk_store.OrderSubmitAction;
import db.DAO;
...
public class Kiosk_barber {//extends JFrame{
	
	JFrame kiosk_barber_frame = new JFrame();
	...
	//예약 소켓통신
	Barber_customer_appointment bca = null;

	
	//사용요금조회
	PaymentAmount pA = new PaymentAmount(255,11,40);
	...
	public Kiosk_barber(){
		...
		kiosk_barber_frame.setTitle("이발");
		kiosk_barber_frame.setBounds(50,50,640,800);
		kiosk_barber_frame.setLayout(null);
		kiosk_barber_frame.addMouseListener(new FrameRefresh());
		
		pA.paymentFF=kiosk_barber_frame;
		...
		for(int i=0; i<arr.length; i++) {
			JButton menuB = new Sep_Button(arr[i],255,11,40);
		...
		kiosk_barber_frame.add(orderP); //주문하기 눌렀을때 보여지는 주문패널
		kiosk_barber_frame.add(showP);  //기본화면에서 매점,사용요금,불만접수화면을 모두 넣은 showP
		kiosk_barber_frame.add(selectP);//매점,사용요금,불만접수화면 버튼을 넣은 하단패널
		
		kiosk_barber_frame.setResizable(false);
		kiosk_barber_frame.setVisible(true);
		kiosk_barber_frame.setDefaultCloseOperation(3);
		...
		storeP.add(bca = new Barber_customer_appointment());
		storeP.repaint();
		storeP.revalidate();
	...
	public void makeProductPane(String str) {//DB에서 카테고리,종류 맞는 메뉴가져오는 메소드
		...
		for(Object ob : DBList) {
			dto = (MenuDTO)ob;
			try {
				if(dto.getSep().equals(str)&&dto.getCategory().equals("2")) {
...
//						ImageIcon producImg = new ImageIcon(changeImg);
						...
						pb = new Menu_Button(dto.getImg(),255,11,40,0,0);
					}else {
						pb = new Menu_Button("No Image",255,11,40);
					}
					/////////////////////////////////////
					...
					pb.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent e) {
							...
							if(paymentList.size()<3) {
								...
								ShowMessage showMsg = new ShowMessage(kiosk_barber_frame,"예약은 최대 3개까지 가능합니다.",1500);
							}
						}
					});
	...
	void orderComp() { // orderListP 에 구성요소 패널들 넣어주는 메소드
		...
		ordFeeP.setBackground(new Color(255,11,40));
		...
		JButton order = new Order_Button("예약하기",245,1,30,25,0);
		order.setForeground(Color.white);
		JButton cancel = new Order_Button("취소하기",245,1,30,25,0);
	...
	void selectedP(MenuDTO _dto) { //제품선택시 선택리스트에 패널 추가하는 메소드
		...
		if(separate==0) { //처음 선택한 제품일때
			...
			JButton delete = new Normal_Button("X",255,11,40,15);
	...
	void order() { //주문하기를 누르면 나오는 화면
		...
		JButton submit = new Menu_Button("예약",255,11,40);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(push.equals("이발")) {
				...
				if(paymentList.size()!=0) {
					...
					new ShowMessage(kiosk_barber_frame,"예약하실 서비스를 선택해 주세요",1500);
				}
	...
	void customerCheck() {
		...
		showMsg.backP = kiosk_barber_frame;
		showMsg.time = 1000;
		...
		if(lockerNum.equals("")||pwNum.equals("")){
			...
			if(lockPass&&PWPass) {
				submitToBarber(lockerNum, pwNum);
				cancel();
	...
	void submitToBarber(String lockerNum, String pwNum) {
		...
		new DAO().barberwaitingInsert(writeAo);
		bca.apmtSender(writeAo);
